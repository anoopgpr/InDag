					 ...
					 for (var i = 0; i < data.data.length; i++)
					 {
                       ...
                       if(showDataless < data.data[i].SNo && showMatData >= data.data[i].SNo){
						 if(data.data[i].MobileNumber.startsWith('91') && data.data[i].MobileNumber.length=='12')
						 {
							...
							// str1 +='<div class="ui-grid-solo" style="margin:0 auto;">';
							// str1 +='<div class="ui-block-a">';
							// str1 +='<input type="text" id="approval_se_txns_id_'+i+'" class="approval_comment" placeholder="Transaction ID*" data-role="none" readonly />';
							// str1 +='</div>';
							// str1 +='</div><br>';
							...
							str1 +='<div class="btmsize">';
							str1 +='<a href="javascript:submitData_txns_se(\'Approved\',\''+i+'\',\''+data.data[i].IHB_MobileNo+'\',\''+data.data[i].BillNumber+'\',\''+data.data[i].MobileNumber+'\',\''+data.data[i].Quantity+'\',\''+data.data[i].brand_code+'\');"  id="complaint_button_se1_'+i+'" class="btn pop_btn btn_primary">Submit</a>'; 
					...
					$.ajax({
						   ...
						   success: function(data, textStatus, xhr) {	
								   ...
								   if(data.sms.length>=1)
								   {
									   ...
									   if(data.sms[0].Status=='Approved')
									   {
											submitData_txns_se_new('Rejected',status,verificationField,reason);
									   }
									   else
									   {
											toast('Site is not approved.');
			...
			else
			{
				 for (var i = 0; i < data.data.length; i++)
				 {
				   ...
				   if(showDataless < data.data[i].sno && showMatData >= data.data[i].sno){
						...
						str1 +='<label style="font-size: 11px;">Verification field<span style="color: red;">*</span></label>';
						str1 +='<div class="select_role_block">';
						str1 +='<select name="approval_se_verification_field_'+ i +'" id="approval_se_verification_field_'+ i +'" data-role="none">';
						...
						str1 +='<div class="select_role_block">';
						str1 +='<select name="approval_se_verification_field_'+ i +'" id="approval_se_rejection_field_'+ i +'" data-role="none">';
						...
						str1+='<div>';
						str1+='<div class="ui-grid-a apr_block"><div class="ui-block-a"><a href="javascript:gotoApprovalpage_leads(\''+i+'\',\''+data.data[i].CustomerId+'\',\''+data.data[i].Mobileno+'\',\''+data.data[i].device_id+'\',\''+data.data[i].device_type+'\',);" class="btn_primary_approve btn_aaprov mt-20 ui-link">Verify</a></div><div id="rejectSubmitShow" class="ui-block-b"><a href="javascript:show_leads_se_reject(\'Rejected'+data.data[i].sno+'\',\''+data.data[i].CustomerId+'\');" class="btn_primary_approve btn_aaprov mt-20 ui-link">Reject</a></div><div id="rejectSubmit" style="display:none" class="ui-block-b"><a href="javascript:submitData_leads_se_reject(\'Rejected\',\''+data.data[i].CustomerId+'\',\''+data.data[i].sno+'\');" class="btn_primary_approve btn_aaprov mt-20 ui-link">Submit</a></div></div>';
						str1 += '</div></div></div>';
						str1 +='</div>';
				   }
				 }
...
function contuSubmore(){
 ...
 function show_leads_se_reject(type,id){
  console.log(type);
  var x = document.getElementById(type);
	if (x.style.display === "none") {
	  x.style.display = "block";
	  $("#rejectSubmit").css("display","block");
	  $("#rejectSubmitShow").css("display","none");
	} else {
	  x.style.display = "none";
	  $("#rejectSubmitShow").css("display","none");
	}
...
function submitData_leads_se(stype)
{
    console.log(stype);
	if(stype=='Approved')
	{
		...
		if(cust_id == ''){
			...
			$.ajax({
				...
				//   "potential_left": cement_leftpotentiall,
				//   "total_LB202_potential": LB202_Potential,
				//   "total_LB202_consumed": LB202_Consumed,
				//   "total_LB202_left": LB202_Left,
				//   "total_LB303_potential": LB303_Potential,
				//   "total_LB303_consumed": LB303_Consumed,
				//   "total_LB303_left": LB303_Left,
				//   "total_AdhesiveXT222_potential": XT22_Potential,
				//   "total_AdhesiveXT222_consumed": XT22_Consumed,
                //   "total_AdhesiveXT222_left": XT22_Left,
				//   "followup": follow_up		 
				},
	...
	else if(stype == 'Rejected')
	{
		...
		// var XT22_Consumed = $('#approval_se_XT22_Consumed').val();
		// var XT22_Left = $('#approval_se_XT22_Left').val();
		console.log(cust_id + ' ' + address + ' ' + state + ' ' + district + ' ' + city + ' ' + reasolnsSitereject + ' ' + verificationField)
		if(cust_id ==''){
			toast('Please enter mandatory fields');
		// } else if(address == ''){
		// 	toast('Please Select Address Field');
		...
		// }else if(city == '' || city == null){
		// 	toast('Please Select City Field');
		// }else if (reasolnsSitereject == '' || reasolnsSitereject==null || reasolnsSitereject=='undefined' || reasolnsSitereject=='null'){
		// 	toast('Please select reason');
		// }else if(verificationField == '' || verificationField == null || verificationField == 'undefined'){
		// 	toast('Please Select Verification Field');
		// }else if(cement_potentiall == '' || cement_potentiall == null || cement_potentiall == 'undefined'){
		// 	toast('Please Select Mendatory Field');
		// }else if(cement_consumed == '' || cement_consumed == null || cement_consumed == 'undefined'){
		// 	toast('Please Select Mendatory Field');
		// }else if(reasolnsSitereject == '' || reasolnsSitereject == null || reasolnsSitereject == 'undefined'){
		// 	toast('Please Select Mendatory Field');
		}else
		{
			$.ajax({
...
function EnroledContractor()
{
	...
	var rcount = $('#update_runningsite_count').val();
		...
		{
			toast('Please enter Firstname fields.');
		}
		else if(mobile.length!='10')
		{
		...
		// {         
		// 	toast('Please enter State fields.');
		// }
		// else if(birthday == '')
	    // {
		//     toast('Please enter mandatory fields.');
		...
		// {       
		// 	toast('Please enter District fields.');
		// }
		// else if(city == '' || city == 'undefined' || city == '')
		// {
		// 	toast('Please enter City fields.');
		// }
